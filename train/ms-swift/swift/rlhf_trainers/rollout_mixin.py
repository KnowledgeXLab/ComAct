...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _move_full_model_to_vllm(self):
        ...
        gather_if_zero3 = get_gather_if_zero3_context(self)
        _dbg_rank = getattr(self.accelerator, 'process_index', 0)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        _dbg_mv_t0 = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        ...
        for i, parameter_group in enumerate(self.parameter_groups):
            ...
            with gather_if_zero3(parameters):
                ...
                try:
                    _dbg_batch_t = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                    ...
                    if _dbg_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                        _n_params = len(parameter_group) if parameter_group else 'all'  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                        print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_rank}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                              f"STAGE=weight_sync_batch batch={i+1}/{len(self.parameter_groups)} "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                              f"n_params={_n_params} elapsed={time.time()-_dbg_batch_t:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                finally:
                    if should_merge:
                        with patch_lora_unmerge(self.model):
                            self.model.unmerge_adapter()
        ...
        if _dbg_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_rank}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"STAGE=move_full_model_to_vllm_total n_batches={len(self.parameter_groups)} "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"elapsed={time.time()-_dbg_mv_t0:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
    ...
    def _colocate_multi_turn_infer(self, inputs: DataType, first_turn_rollout_outputs: List[RolloutOutput],
        ...
        _dbg_mt_rank = getattr(self.accelerator, 'process_index', 0)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        _dbg_mt_t0 = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        while True:
            ...
            _dbg_turn_t = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            outputs = self._rollout(infer_requests if has_local_data else [], request_config)
            if _dbg_mt_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_mt_rank}][step={getattr(self, 'state', None) and self.state.global_step}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                      f"STAGE=multi_turn_rollout turn={current_turn} "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                      f"n_active={len(infer_requests)} elapsed={time.time()-_dbg_turn_t:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            index_to_infer = next_turn_index_to_infer

        if _dbg_mt_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_mt_rank}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"STAGE=colocate_multi_turn_total turns_executed={current_turn-1} "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"elapsed={time.time()-_dbg_mt_t0:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
    ...
    def _fast_infer(self, inputs: DataType) -> DataType:
        ...
        _dbg_rank = getattr(self.accelerator, 'process_index', 0)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        _dbg_t0 = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        ...
        _dbg_sync_t = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        if self.state.global_step != self._last_loaded_step or args.sleep_level == 2:
            self._move_model_to_vllm()
            self._last_loaded_step = self.state.global_step
        if _dbg_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_rank}][step={self.state.global_step}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"STAGE=weight_sync_to_vllm elapsed={time.time()-_dbg_sync_t:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        ...
        with context():
            ...
            if hasattr(self, 'async_generate') and self.async_generate:
                ...
                _dbg_gen_t = time.time()  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                with self.multi_turn_completion_length_context():
                    outputs = self._infer_single_or_multi_turn(inputs, self.request_config)
                if _dbg_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                    print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_rank}][step={self.state.global_step}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                          f"STAGE=vllm_multiturn_generation elapsed={time.time()-_dbg_gen_t:.2f}s "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                          f"n_samples={len(inputs)}", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        ...
        if _dbg_rank == 0:  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
            print(f"[DEBUG:swift/rlhf_trainers/rollout_mixin.py][rank={_dbg_rank}][step={self.state.global_step}] "  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
                  f"STAGE=fast_infer_total elapsed={time.time()-_dbg_t0:.2f}s", flush=True)  # [DEBUG:swift/rlhf_trainers/rollout_mixin.py]
        return outputs
